function [freq,freqspect,Time] = lfpf_prepDataFFT3(Signal, Fs, TimeWindow, Overlap)
%
% [freq,freqspect,Time] = lfpf_prepDataFFT3(Signal, Fs, TimeWindow, Overlap)
...
for k = 1:nFFTChunks
   ...
   [freq, npower] = lfpf_FFT3(Signal, k*winstep/Fs, TimeWindow, Fs, 1, 1, 0);
   for l = 1:nChannels
       freqspect(:,k,l) = npower(:,l);
   end
end
close(h)
toc