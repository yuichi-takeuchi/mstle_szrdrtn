function [ flag ] = figsf_2ANOVAOpt1( sBasicStats, dataVarNames, condVec, control, CTitle, CVLabel, outputGraph, outputFileNameBase)
%
...
for i = 1:length(dataVarNames)
    ...
    set(hs.heb(1),  'Color', 'k'); % 'YNegativeDelta',[],
    set(hs.heb(2),  'Color', 'b'); % 'YNegativeDelta',[],
%     set(hs.heb(2), 'Color', [0.5 0.5 0.5]);
    set(hs.hylabel, 'String', CVLabel{i});
    if(control)
        set(hs.hxlabel, 'String', 'MS illumination delay (ms)'); 
    else
        set(hs.hxlabel, 'String', 'MS illumination frequency (Hz)');
    end
