function [ flag ] = lfpprepf_Takeuchi5_Amplipex_with1D( datfilenamebase, sr, srLFP, nChannels)
%
% [ flag ] = lfpprepf_Takeuchi5_Amplipex_with1D( datfilenamebase, sr, srLFP, nChannels)
%
% This function resamples, DC removes, extracts dig ch from the
% original dat file
%
% INPUTS:
...
apf_ExtractChannels([datfilenamebase '_DSampled.dat'],...
                    ...
                    4 14],...       % vHPC
                    nChannels);
disp('extracting HS channels done.')
                
% Remove DC from analog channels file
disp('removing DC shifts from analog channels...')
[~,~] = RemoveDCfromDat([datfilenamebase '_reorg.dat'], (nChannels-1));
